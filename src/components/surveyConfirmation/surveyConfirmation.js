...
import {
  ...
  ConfirmationContent,
  TextContainer
} from "./surveyConfirmation.styles";
...
const SurveyConfirmation = ({ animName, animId }) => {
  ...
  return (
    <>
      <H1Light>
        Your <AccentText color={getColorCode(animId)}>{animName}</AccentText>{" "}
        manifestation
      </H1Light>
      ...
      <ConfirmationContent>
        <ImageContainer>
          <PreviewImage
            src={animPreviews[animId]}
            alt={`Animation screenshot for ${animName}`}
          />
        </ImageContainer>
        <TextContainer>
          <div>
            <H2>
              Curious about how we selected this color based off of your input?
            </H2>
            <Link to="/research">
